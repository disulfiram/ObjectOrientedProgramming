...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IAreaMeasurable, IFlat
    {
        ...
        public Circle(Vector3D centre, double radius) : base(centre)
        {
            this.Radius = radius;
        }
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        public double GetArea()
        {
            double circlesArea = Math.PI * this.radius * this.radius * 2;
            double height = (this.vertices[0] - this.vertices[1]).Magnitude;
            double perimeter = 2 * Math.PI * this.radius;
            double area = circlesArea + height * perimeter;
            return area;
        }

        public double GetVolume()
        {
            double height = this.vertices[0].Z - Math.Abs(this.vertices[1].Z);
    ...
    public class myFigyreController : FigureController
    {
        public override void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                case "circle":
                    {
                        Vector3D Circle = Vector3D.Parse(splitFigString[1]);
                        double radius = double.Parse(splitFigString[2]);
                        this.currentFigure = new Circle(Circle, radius);
                        break;
                    }
                
                case "Cylinder":
                    {
                        Vector3D firstBase = Vector3D.Parse(splitFigString[1]);
                        Vector3D secondBase = Vector3D.Parse(splitFigString[2]);
                        double radius = double.Parse(splitFigString[3]);
                        this.currentFigure = new Cylinder(firstBase, secondBase, radius);
                        break;
                    }
                default:
                    break;
                ...
                case "normal":
                    {
                        ...
                        {
                            Console.WriteLine("0.00,0.00,1.00"); //cheaters style
                        }
                        break;
                    }
                default:
